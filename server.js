...
function formatNewsMessage(news){
  ...
  const lines = items.map((n,i)=>{
    const title = n.title || '新聞標題未取得';
    const source = n.source || '新聞來源';
    const published = n.publishedAt ? `｜發布：${taipeiTimeText(n.publishedAt)}` : '';
    const summary = n.summary || '請點選連結查看完整內容。';
    const url = n.url || 'https://news.google.com/search?q=%E5%99%AA%E9%9F%B3%E8%BB%8A%20%E8%81%B2%E9%9F%B3%E7%85%A7%E7%9B%B8&hl=zh-TW&gl=TW&ceid=TW:zh-Hant';
    return `■ ${i+1}. ${title}\n來源：${source}${published}\n摘要：${summary}\n連結：${url}`;
  }).join('\n\n');
...
app.get('/healthz', (req,res)=>res.json({ok:true, service:'newtaipei-noise-control-system-v28-news-link', hasAdminPassword:!!ADMIN_PASSWORD, hasSessionSecret:!!SESSION_SECRET}));
...
function richMenuSpec(){
  return { size:{width:2500,height:1686}, selected:true, name:'新北噪音車V28新聞連結版圖文選單', chatBarText:'管理選單', areas:[
...
app.listen(PORT, ()=> console.log(`New Taipei V28 News Link running on :${PORT}`));
