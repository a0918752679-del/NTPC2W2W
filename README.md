...
```json
{"ok":true,"service":"newtaipei-noise-control-system-v28-news-link"}
```
...
- LINE Bot 輸入「新聞」、「噪音車新聞」、「更新新聞」後，新聞回覆內容會直接附上每則新聞連結。
- 若新聞來源未提供 URL，系統會補上 Google News 噪音車／聲音照相搜尋連結。
- 三平台 Google Sheet 分流設定沿用 V27，不需變更環境變數。
